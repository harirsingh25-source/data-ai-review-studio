...
test_auc = roc_auc_score(y_test, prob)""",
    ...
    Stage(
        "Prediction arena",
        "Now that model selection is finished: is your custom Pokémon Legendary, and do the two finalist models agree?",
        "The same input can produce different model scores because KNN and Random Forest learn different decision rules. The selected winner should be identified by validation evidence, not by whichever answer feels more exciting.",
        ("tuned KNN score", "tuned Random Forest score", "whether the finalists agree", "which model won the tournament"),
        "A prediction is an output from a fitted model, not proof that the underlying label is true. Treat model scores as evidence with known validation context.",
        """with st.form("prediction_arena"):
    # collect all six model features
    submitted = st.form_submit_button("Run prediction")

if submitted:
    # compare the tuned finalists on the same Pokémon
    results = predict_with_finalists(df, features, summary)""",
    ),
]
