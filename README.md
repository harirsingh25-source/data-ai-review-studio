...
### Kahoot structure for tomorrow

The workbook now has two layers:

1. **Core review questions** — cross-validation, leakage, GridSearchCV, final-test discipline and one new Streamlit concept.
2. **Rapid-fire future bridge** — one question each on Transformers, CNNs, RNNs, Time Series, MLflow, model serving, GCP, A/B testing and LightGBM.

Use the bridge questions only if time allows, or select a few as the closing Kahoot round.
